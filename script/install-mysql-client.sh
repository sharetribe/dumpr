...
DEB="mysql-apt-config_0.8.28-1_all.deb"

apt-get update --allow-releaseinfo-change && apt-get install -y lsb-release wget gnupg

wget -O "/tmp/$DEB" "http://repo.mysql.com/$DEB"

# The repo config package can configure any supported version of MySQL. We need
# to select 8.0. The package uses debconf, so we need to pass the correct
